...
model.component('comp1').physics('spf').feature('BufferInl').set('p0', BufferInlet);  
...
model.component('comp1').physics('spf').feature('MainInl').set('p0', 100);
...
for i=1:1:BumpNumber
    ...
    model.component('comp1').geom('geom1').feature("BI"+i).set('ymax', 50+BumpDiameter+BumpPeriod*(i-1)+1+OutletWidth(i));
...
model.result('pg1').feature('str1').set('xcoord', 49.9);
model.result('pg1').feature('str1').set('ycoord', 100);
model.result('pg1').run;
pg=mphplot(model,'pg1');
